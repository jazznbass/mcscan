...
table_mc_tau <- function(data_mc,
  ...
  if (reference_category == 0) {
    for(i in 3:10) df[[i]] <- round(df[[i]], 2)
  }

  if (reference_category == 1) {
    df[, c(3, 7)] <- round(df[, c(3, 7)], 2)
    for(i in 4:6) df[[i]] <- round(df[[i]] - df[[2+reference_category]], 2)
    for(i in 8:10) df[[i]] <- round(df[[i]] - df[[6+reference_category]], 2)
  }
